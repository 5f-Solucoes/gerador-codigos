                ...
                <form method="POST" action="{{ route('clientes.update', $cliente->id) }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <x-text-input id="nome_fantasia" class="block mt-1 w-full uppercase" type="text" name="nome_fantasia" :value="old('nome_fantasia', $cliente->nome_fantasia)" required />
                        </div>

                        <div>
                            <x-input-label for="razao_social" :value="__('Razão Social')" />
                            <x-text-input id="razao_social" class="block mt-1 w-full uppercase" type="text" name="razao_social" :value="old('razao_social', $cliente->razao_social)" />
                        </div>
                    </div>
                    ...
                    <div x-data="{
                        // AQUI ESTÁ O TRUQUE: Convertemos a collection do Laravel para JSON
                        filiais: {{ $cliente->filiais->count() > 0 ? $cliente->filiais->toJson() : '[{cnpj: \'\', localidade: \'\'}]' }},
                        add() {
                        ...
                        remove(index) {
                            if(this.filiais.length > 1) {
                                ...
                                alert('É necessário ter pelo menos uma filial.');
                            }
                        }
                    }">
                            ...
                            <template x-for="(filial, index) in filiais" :key="index">
                                <div class="flex gap-4 items-start">
                                    <div class="w-1/2">
                                        ...
                                        <input type="text" :name="'filiais['+index+'][cnpj]'" x-model="filial.cnpj" required
                                    ...
                                    <div class="w-1/2">
                                        <x-input-label :value="__('Localidade')" />
                                        <input type="text" :name="'filiais['+index+'][localidade]'" x-model="filial.localidade" required
                                    ...
                                    <div class="pt-6">
                                        <button type="button" @click="remove(index)" class="text-red-500 hover:text-red-700">
